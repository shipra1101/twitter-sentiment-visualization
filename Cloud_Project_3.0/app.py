...
def trend_Analysis():
    ...
    if(len(result)>1):
        ...
        for row in result:
            ...
            neutral.append(float(row[4]))
        raw_data = {'greenBars': positive, 'orangeBars': negative,'blueBars': neutral}
        r = np.arange(len(positive));
        df = pd.DataFrame(raw_data)
        barWidth = 0.85
        greenBars = df['greenBars']
        orangeBars = df['orangeBars']
        blueBars = df['blueBars']
        names = ('A','B','C','D','E')
        plt_trend.xticks(r, r)
        plt_trend.xlabel('Searches')
        plt_trend.ylabel('percentage') 
        plt_trend.bar(r, greenBars, color='#66ff33', edgecolor='white', width=barWidth)
        plt_trend.bar(r, orangeBars, bottom=greenBars, color='#F7464A', edgecolor='white', width=barWidth)
        plt_trend.bar(r, blueBars, bottom=[i+j for i,j in zip(greenBars, orangeBars)], color='#FDB45C', edgecolor='white', width=barWidth)
        plt_trend.legend(['Positive', 'Negative', 'Neutral'], loc='upper left')
        img_trend = io.BytesIO()
        plt_trend.savefig(img_trend, format='png', dpi=200)
        img_trend.seek(0)
